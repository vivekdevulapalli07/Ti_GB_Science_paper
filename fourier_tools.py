...
def fft2_blur_peaks(image, gauss_sigma: float, hann: bool):
    ...
    fft2_blur = (fft2_blur - np.min(fft2_blur)) / ((np.max(fft2_blur) - np.min(fft2_blur)))
...
def fft2_blur(image, gauss_sigma: float, hann: bool, crop: float, scale: bool, cont_boost: float):
    ...
    # Apply transform if region is non-square
    (Ny, Nx) = fft2.shape
    if Nx>Ny:
        tform = np.array([[Nx/Ny, 0, 0],[0, 1, 0],[0, 0, 1]])
        fft2 = warp(fft2, tform, output_shape=(Ny, Ny))
    elif Ny>Nx:
        tform = np.array([[1, 0, 0],[0, Ny/Nx, 0],[0, 0, 1]])
        fft2 = warp(fft2, tform, output_shape=(Nx, Nx))
